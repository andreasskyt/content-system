/**
 * LandscapeCamEngland — subtitles that stay subtitles, plus depth headers.
 *
 * Tier 1, subtitles: Inter, lowercase, one weight, one size, lower-third.
 *   Built word by word — each word fades and rises into a pre-allocated slot,
 *   so words already on screen never shift. Calm pacing.
 * Tier 2, depth headers: massive ALL-CAPS in the top half, BEHIND the speaker
 *   via a segmentation mask. Rendered by scripts/depth.ts.
 *
 * There is deliberately NO inline emphasis tier. A word promoted to ALL-CAPS
 * mid-sentence breaks the flow of the caption it belongs to and competes with
 * the header tier for the same job. Emphasis lives in the header, above and
 * behind — never inside the subtitle line. (Set 2026-08-25 from review.)
 *
 * Everything is #FFFFFF at full opacity, no shadow, no outline.
 */
...
const style: EditStyle = {
  meta: {
    name: "LandscapeCamEngland",
    description:
      "16:9 captions: lowercase Inter subtitles built word by word in the lower third, plus massive ALL-CAPS depth headers behind the speaker. No inline emphasis.",
  },
  ...
  subs: {
    ...
    // 700. 500 was tried and read as too light on screen at this size.
    ...
    depth: {
      ...
      min_font_size_pct_height: 13,
      max_font_size_pct_height: 24,
